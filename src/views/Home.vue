<template>
  <div class="pb-5">
    <!--HEADER BIENVENIDA A LA PAG-->
      <div class="container-portada">
        <div class="capa-gradient">
        </div>
        <div class="container-details">
            <div class="details pt-5">
                <h1>The Cocktail</h1>
               <p><strong>Ingresa y selecciona tus tragos favoritos</strong></p>
               
                <button round><router-link :to="{ name: 'Registro' }"><strong>REGISTRATE</strong></router-link></button>
                
            </div>
        </div>
      </div>


    <!-- <div class="pt-5">
      <b-jumbotron class="jumbo" header="The Cocktail" lead="">
        <p>Ingresa y selecciona tus tragos favoritos</p>

        <el-button class="boton" round><router-link :to="{ name: 'Registro' }">REGISTRATE</router-link></el-button>
      </b-jumbotron>
    </div> -->

    <!--BODY-->
    <div class="pt-5">

    <Carousel />
      <h1>algunas card y carousel</h1>

      <b-container class="p-1 pt-5">
        <b-row>
          <b-col sm="6">
            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aliquam at sunt tempore itaque cum reiciendis dicta veritatis fuga est omnis ea quidem, tenetur blanditiis. Totam sunt a accusamus fugiat aliquid.</p>
          </b-col>
          <b-col sm="6">
            <img src="../assets/img/drink6.jpg" fluid alt="Fluid image" rounded="top"></img>
          </b-col>
        </b-row>
        <b-row class="pt-5">
        <b-col sm="6" md="6">
        <img src="../assets/img/drink1.jpg" alt="">
            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aliquam at sunt tempore itaque cum reiciendis dicta veritatis fuga est omnis ea quidem, tenetur blanditiis. Totam sunt a accusamus fugiat aliquid.</p>
          </b-col>
          <b-col sm="6" md="6">
        <img src="../assets/img/drink3.jpg" alt="">
            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aliquam .</p>
          </b-col>
        </b-row>
      </b-container>
    </div>
  </div>
</template>
<script>
import Carousel from '../components/Carousel'
export default {
  name: "Home",
  components: {
    Carousel
  }
};
</script>
<style scoped lang="scss">
*{
    margin: 0;
    padding: 0;
    box-sizing: border-box;
    font-family: 'Raleway', sans-serif;
}

// .jumbo {
//   background-image: url("../assets/img/barra1_1200.jpg");
//   background-position: center top cover;
//   color: white;
// }
// .boton {
//   text-decoration: none;
// }
// .boton:hover {
//   text-decoration: none;
// }
.container-portada{
    width: 100%;
    height: 450px;
    position: relative;
    background-image: url(../assets/img/barra1_1200.jpg);
    background-size: 130%;
    animation: movimiento 20s infinite linear alternate;  
}

@keyframes movimiento{
    from{
        background-position: bottom left;
    }to{
        background-position: top right;
    }
}

.capa-gradient{
    width: 100%;
    height: 100%;
    position: absolute;
    background: -webkit-linear-gradient(left, black, #0672d0);
    opacity: 0.5;
}

.container-details{
  padding-top:4em ;
    width: 100%;
    max-width: 1200px;
    position: relative;
    margin: auto;
}


.details{
    width: 100%;
    position: relative;
    top: 20px;
    color: white;
}

.details h1{
    font-size: 40px;
    font-weight: 700;
    margin-left: 10px;
}

.details p{
    margin-top: 10px;
    font-size: 20px;
    font-weight: 500;
    margin-left: 10px;
}

.details button{
    padding: 10px 20px;
    font-size: 16px;
    background: none;
    border-style: none;
    border: 1px solid white;
    color: white;
    margin-top: 20px;
    transition: background 300ms;
    cursor: pointer;
    margin-left: 10px;
    border-radius: 20px;
    background: white;
}

.details button:hover{
    background: rgb(236, 221, 221);
    color: black;
    text-decoration: none;
    
}


</style>
